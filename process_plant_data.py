"""
process_plant_data.py — Mountain View Shopping Centre

Processor for Mountain View Shopping Centre PV monitoring dashboard:
- Stores historical daily data (rolling window)
- Fetches irradiation from Open-Meteo API
- Calculates rolling hourly averages (bell curve baseline)
- Calculates min/max bands and percentiles (p10/p25/p75/p90)
- Sends Telegram alerts for underperformance
- Produces dashboard-ready JSON

All thresholds are purely data-driven — no hardcoded expected/low values.
The system gracefully handles the bootstrap period (< 7 days of data)
by suppressing alerts until enough history has accumulated.
"""
...
# ✏️  SITE CONFIG
# =============================================================================
PLANT_NAME = os.environ.get("PLANT_NAME", "Mountain View Shopping Centre")

# PV Yield column fallback — 0-based (A=0, B=1, C=2, D=3, E=4, F=5...)
# Auto-detected from headers; this is only a last-resort fallback
PV_COLUMN_INDEX = 4

# Site location for irradiation (Gqeberha / PE area)
SITE_LATITUDE  = -33.97422273793887
SITE_LONGITUDE = 25.61212584301634
...
# FIXED CONFIG
# =============================================================================
PACE_THRESHOLD_PCT = 0.30     # alert if actual < 30 % of curve-expected
OFFLINE_THRESHOLD  = 0.01     # kWh — treat as offline below this
HISTORY_DAYS       = 30       # rolling window
MIN_HISTORY_DAYS   = 7        # suppress alerts until this many sample days
REPORT_UTC_OFFSET  = 2        # FusionSolar reports are UTC; +2 for SAST
...
def solar_window(month: int) -> tuple:
    """Seasonal sunrise/sunset approximation for ~33.8°S."""
...
def calculate_30day_stats(history: dict, exclude_date: str = None) -> dict:
    """
    Calculate rolling statistics from accumulated history.
    Returns zeros when no data — the dashboard and alerts handle this gracefully.
    """
    empty = {
        "hourly_avg": [0] * 24,
        "hourly_min": [0] * 24,
    ...
    for h in range(24):
        ...
        if not vals:
            ...
            hourly_min.append(round(min(nonzero), 2) if nonzero else 0)
...
# =============================================================================
# Status checks — purely data-driven
# =============================================================================

def determine_status(data: dict, month: int, stats: dict, irradiation: list = None) -> tuple:
    """
    Returns (status, alerts, debug).
    All thresholds derived from rolling history.
    Suppresses alerts when < MIN_HISTORY_DAYS of data.
    """
    ...
    # Not enough history — report ok, skip threshold checks
    if sample_days < MIN_HISTORY_DAYS:
        return "ok", alerts, {
            "reason": f"insufficient history ({sample_days}/{MIN_HISTORY_DAYS} days)",
            "curve_fraction": round(curve_frac, 3),
            "expected_by_now": 0.0,
            "pace_trigger": 0.0, "projected_total": 0.0,
            "irrad_factor": 1.0,
    ...
    # ── Dynamic thresholds from history ───────────────────────────
    ...
    if irradiation and stats.get("hourly_irrad_avg"):
        ...
        if avg_cum > 0:
            ...
            print(f"  🌤️  Irrad factor: {irrad_factor:.2f} "
                  f"(today {today_cum:.0f} vs avg {avg_cum:.0f} W/m² cumulative)")
    ...
    # Check 2: projected total below rolling minimum
...
def send_alerts(status: str, alerts: dict, data: dict, debug: dict):
    ...
    daily_min       = debug.get("daily_min", 0)
    sample_days     = debug.get("sample_days", 0)

    # Suppress non-offline alerts during bootstrap
    if sample_days < MIN_HISTORY_DAYS and not alerts["offline"]:
        print(f"  ℹ️  Alerts suppressed — only {sample_days}/{MIN_HISTORY_DAYS} days of history")
        return
    ...
    if alerts["offline"]:
        ...
        if alerts["total_low"]:
            send_telegram(
                ...
                f"At current pace, today will finish below the rolling minimum.\n"
                f"Actual so far:      <b>{total:.1f} kWh</b>\n"
                f"Projected end-day:  <b>~{projected_total:.0f} kWh</b>\n"
                f"Rolling minimum:    <b>{daily_min:.0f} kWh</b>\n"
                f"Hour: {hour:02d}:00 | 🕐 {now_str}"
            )
...
def main():
    ...
    print(f"  📈 Rolling avg:        {stats['daily_avg']:.1f} kWh  "
          f"(min: {stats['daily_min']:.1f}, max: {stats['daily_max']:.1f})")
    ...
    output = {
        ...
        # Rolling statistics (for chart baselines)
        "stats_30day": stats,

        # Historical data (for export/filtering)
        "history": history,
